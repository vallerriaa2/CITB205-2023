...
void TextPrinter::print(std::ostream &out, Invoice &invoice)
{
    ...
    {
        int c = 0;
        const Product *product = invoice.productAt(i);
        out << '|';
        for (int colIdx = 0; colIdx < columns.size(); colIdx++)
        {
            out << setw(columns[colIdx]);
    ...
    out << setw(summaryOffset) << "";
    separator(out, totalsColumns);
    out << setw(summaryOffset) << "Discounts" << '|' << setw(lastColumn) << -invoice.discount() << '|' << endl;
    out << setw(summaryOffset) << "";
    separator(out, totalsColumns);
